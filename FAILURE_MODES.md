...
| Delete running job | `DELETE /jobs/{id}` while status is `running` | API returns `409` JSON conflict; running pod is not orphaned by DB row removal | Wait for terminal status before delete | Unit + integration tests for delete conflict |
| DB outage during readiness | Database unavailable during readiness/health checks | `/readyz` and `/health` return `503`, removing instance from traffic | Restore DB connectivity, readiness returns `200` | Unit tests for readiness + health status codes |
...
| Submission abuse burst | Single client exceeds `POST /jobs` rate policy | API returns `429` JSON response while remaining healthy for other clients | Client retries later with backoff | Unit tests for rate limiting |
